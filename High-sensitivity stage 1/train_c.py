...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            imgn_train = add_cloud_noise( img_train, occupancy=50,alpha=opt.alpha)
            if opt.self_supervised == "True":
                #imgn_train_2 = add_watermark_noise(img_train, 40, True, random_img, alpha=opt.alpha)
                imgn_train_2 = add_cloud_noise(  img_train,  occupancy=50, alpha=opt.alpha )
            else:
            ...
            if step % 10 == 0:
                ...
                gt_show = vutils.make_grid(
                    img_train.detach().cpu().clamp(0, 1),
                    normalize=False
                )

                input1_show = vutils.make_grid(
                    imgn_train.detach().cpu().clamp(0, 1),
                    normalize=False
                )

                input2_show = vutils.make_grid(
                    imgn_train_2.detach().cpu().clamp(0, 1),
                    normalize=False
                )

                output_show = vutils.make_grid(
                    out_train.detach().cpu().clamp(0, 1),
                    normalize=False
                )
                ...
                difference = torch.abs(
                    imgn_train - imgn_train_2
                )

                difference_show = vutils.make_grid(
                    difference,
                    normalize=True,
                    scale_each=True
                )
        ...
        with torch.no_grad():
            for k in range(len(dataset_val)):
                ...
                imgn_val = add_cloud_noise(  img_val,  occupancy=50, alpha=opt.alpha )
