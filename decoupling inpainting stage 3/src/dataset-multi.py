...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        if size != 0:
            h_idx = random.randint(0, self.pos )
            w_idx = random.randint(0, self.pos )
    ...
    def load_mask(self, img, index):
        ...
        #mask_type =  2
        #print(f"\nmask type: {mask_type}", flush=True)
        ...
        if mask_type == 6:
            ...
            # New: Multi-line / tilted linear missing region mask (Type 7)
        if mask_type == 3:
            return create_line_mask(
                width=imgw,
                height=imgh,
                num_lines=random.randint(2, 5),  # Random 2 to 5 lines
                line_width_range=(70, 100),
                angle_range=(0, 180),  # Full angle range
                gap_range=(15, 40),
                randomize=True
            )
        
        # New: Multi-line mask with fixed direction (Type 4)
        if mask_type == 4:
            # Fixed to horizontal or vertical direction
            ...
            return create_line_mask(
                ...
                angle_range=(angle, angle),  # Fixed angle
                gap_range=(20, 60),
                randomize=False
            )

            # Random irregular
            # New: Fixed-direction multi-line mask (Type 8)
